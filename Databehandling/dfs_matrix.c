...
int dfs_matrix(int startnode, int n, const int adj[n][n], int dfsmatrix[], int visited[], int parent[],int cycles[][2], int *cycle_count, int count) {
    ...
    for (int i = 0; i < n; i++) {
        if (adj[startnode][i] >= 1 && visited[i] != 1) {
            ...
            parent[i] = startnode;
            count = dfs_matrix(i, n, adj, dfsmatrix, visited, parent, cycles, cycle_count, count);
        }
		...
		int already_seen = 0; 
			...
			if (already_seen == 0){
				...
				printf("Cycle detected: node %d, to node %d \n", cycles[*cycle_count][0], cycles[*cycle_count][1]);
				*cycle_count += 1; 
